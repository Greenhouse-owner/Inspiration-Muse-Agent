...
## 用户使用 Tips · 看到这些提示该怎么办

Muse 在使用过程中可能会弹出一些括号里的提示。这里把它们分成三类，看清是什么再决定下一步。

### 🟢 不是错——只是状态提示

| 提示 | 意思 | 该做什么 |
|------|------|---------|
| 正在生成… | Muse 在跑 AI，几秒到十几秒 | 等一下，期间输入框暂时锁住 |
| 词够了，可以生成 ✦ | 选词数量已达门槛 | 点"生成"，或继续选词加细节 |
| 还需选 N 个词 | 当前选的词不够触发生成 | 多选 N 个再生成 |
| 「AI 暂时离线，词卡为本地生成」（橙色小字） | AI 临时不可达，词卡是本地兜底版 | 还能用，5-30 秒会自动恢复，恢复后提示自动消失 |

### 🟡 AI 服务出了小岔子——多半重试就好

这一类是网关临时故障（502 / 超时 / 限流），系统已经自动重试过一次还失败。等几十秒重新触发就行。

| 提示 | 推荐动作 |
|------|---------|
| 章节生成失败了，网络可能波动 | 等 10 秒，重新输入"分 N 章" |
| 插入章节没成功，服务暂时忙不过来 | 等一下，重新点"+ 在 X 章后插入" |
| 修改请求没送出去，网络不太稳 | 等 30 秒，重新输入修改需求 |
| 修改失败了，可能是服务暂时过载 | 等半分钟再发一次 |
| AI 暂时连不上，先用本地模板垫一版 | 先看着用，几分钟后点"换一批"或重新生成可拿到 AI 真版本 |

### 🔴 是操作问题——需要调整你的输入

| 提示 | 你做了什么 | 该怎么改 |
|------|----------|---------|
| 拆章节需要先有故事 | 在角色/世界观路径下输了"分 N 章" | 切到"故事梗概"路径，生成故事后再拆 |
| 已经 20 章了，先删掉再插入新的 | 章节达到上限 | 用"删除"按钮去掉一章再插 |
| Muse 没听懂要改哪里，试试说得更具体 | refine 输入太抽象（"改一下"、"再试试"） | 写具体方向：「把结局改成开放式」、「主角性格再阴郁一些」 |
| 章节数请填 1-20 | 输入了 0 章或 25 章 | 输 1-20 之间的整数 |

### 几个不会弹提示但容易困惑的点

- **输入只有 1 个字（"嗯""好"）回车没反应**：太短不触发 AI，至少 2 个字
- **选词到 20/20 不能再加**：先点已选词右上的 ✕ 删几个
- **章节"删除"按钮**：纯本地操作，不会失败，序号会即时重排
- **每个 AI 输出卡片右下角的淡淡的 "copy"**：点一下复制成纯文本，方便贴到 Notion / 微信 / 文档
